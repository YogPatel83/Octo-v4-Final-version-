export async function executeLogisticsAdapter(input: {
  provider: string;
  action: string;
  ...
  if (!endpoint) {
    return {
      ...
      status: "failed",
      output: null,
      error: "endpoint or base_url is required for logistics adapter.",
    };
  }
  ...
  const res = await fetch(endpoint, {
    ...
    body: method === "GET" ? undefined : JSON.stringify(input.payload.body || input.payload),
  });
